public class Solution {
    ...
    public List<int[]> kSmallestPairs(int[] nums1, int[] nums2, int k) {
        ...
        for (int i=0; i<nums1.length && i<k ; i++) { // only need first k number in nums1 to start  , just add the first set of numbers 
            q.offer( new Pair(0, nums1[i],nums2[0]) );
        }
        for (int i=1; i<=k && !q.isEmpty(); i++) { // get the first k sums
            Pair p = q.poll(); // whenever we pop a number add another pair, same number from num1 and the next number from num2
            ret.add( p.pair );
